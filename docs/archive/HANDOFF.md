...
### 2026-08-14 — Pasada completa: TODO cerrado, dos bugs intermitentes cazados, Infinitas sin tinte, el Archivero en movimiento y ocho dispositivos nuevos

Sesión autónoma sobre el TODO completo + cinco pedidos directos + un
encargo a mitad de sesión (ocho dispositivos nuevos). Todo verificado
contra herramientas en esta sesión: tsc/eslint/next build limpios, suite
Playwright propia 15/15, capturas claro/oscuro revisadas a ojo.

**Bug del héroe de portada (intermitente) — causa raíz encontrada:**
`NewsGrid.selectSource` comparaba el clic contra `activeSource`
COMMITEADO mientras un fade a otra fuente seguía en vuelo (el estado
solo se commitea en el `onComplete` del fade-out), así que un
"Todo → Infinitas → Todo → Infinitas" rápido se tragaba el último clic y
la grilla quedaba en el héroe de Noticias; además los tweens solapados
commiteaban en orden de terminación, no de clic. Fix determinista, sin
delays: guard contra `pendingSource`, un solo tween en vuelo que cada
clic mata (un tween muerto jamás dispara su onComplete, así que una
selección superada no puede commitear), y el fade-in encadenado en el
mismo onComplete (volver a la fuente ya commiteada no re-renderiza, y un
effect con `[activeSource]` habría dejado la grilla en opacity 0).
Verificado con tormentas de clics en Playwright: el último clic gana
siempre, opacity termina en 1.

**Bug del toggle de tema (intermitente) — causa raíz doble:** (1) el
botón se server-renderiza y su onClick de React no existe hasta hidratar
— en cargas lentas cada tap en esa ventana moría en silencio; (2)
hallazgo más profundo, probado bloqueando `/_next/static/chunks/*` en
Playwright: en App Router los `<Script strategy="beforeInteractive">`
inline NO van inline — se serializan en `self.__next_s` y los ejecuta el
bootstrap del framework, así que el script de tema tampoco corría antes
del primer paint (FOUC latente). Fix: el script de tema ahora es un
`<script dangerouslySetInnerHTML>` crudo que además ES el dueño del
toggle (listener delegado en `[data-theme-toggle]` + `__pbToggleTheme`),
funcionando antes y después de la hidratación; React solo maneja aria y
cerrar el drawer (llamar toggleTheme ahí duplicaría el toggle en no-op).
theme-store escucha `playbook:theme-change` para el estado aria.
Verificado pre-hidratación (chunks bloqueados: el toggle cambia
data-theme) y post (un flip por clic, persistencia, aria).

**TODO #2 — `industry-shots` → `noticias`, lado código completo:**
KNOWN_SOURCES/SOURCE_LABELS/taxonomy/product-hubs/schema default/páginas/
API/CMS/ShotProgress/6 CSS renombrados; `normalizeSource()` en
lib/constants.ts aplicado UNA vez en el data boundary
(lib/data/articles.ts) mapea filas legacy en lectura, así que el sitio es
correcto contra ambos estados de la base (y
`/archivo?source=industry-shots` viejo sigue filtrando). **PENDIENTE, un
solo paso POST-deploy:** `POSTGRES_URL=<prod> npx tsx
scripts/migrate-source-noticias.ts` (93 filas legacy verificadas por
--dry-run contra la base real; correrlo ANTES del deploy vaciaría
/noticias porque el build desplegado matchea el string viejo — el script
lo documenta, con rollback).

**TODO #1 — clasificación:** vocabulario controlado ejecutado.
`validateTags()`/`canonicalizeTag()` en lib/taxonomy.ts (fold de
mayúsculas/acentos/espacios, rechazo con sugerencia de opción más
cercana), aplicado en scripts/publish-newsletter.ts (falla el publish),
app/api/update-articles/route.ts (canonicaliza, descarta y reporta
`droppedTags`) y lib/actions/admin.ts (backstop). Auditoría contra la
base real: 133 filas, cero fuera de vocabulario, cero tiers vacíos — no
hubo backfill que hacer. `scripts/audit-taxonomy.ts` queda para re-correr.
`priority` queda editorial a propósito.

**TODO #3:** borrados scripts/update-matador-report.ts y
articles.json/content.json (el riesgo era re-sembrar la base por
accidente; todo recuperable de git). Prototipos HTML e isotope-dark se
quedan — el propio TODO los reserva a decisión humana. docs/TODO.md
reescrito con el estado real de los tres ítems.

**Bug del apóstrofo (HANDOFF 2026-08-06) — cerrado en el escapeo:**
lib/html-entities.ts nuevo (escapeHtml + decodeEntities, el contrato
decode-then-escape); article-devices lo importa, y los tres huecos que
seguían vivos quedaron cerrados: parseMap doble-escapaba etiquetas,
parseCifra/parseJugada pasaban entidades crudas a atributos (una comilla
rompía el aria-label) y a las cards de hub como texto literal. Probado
con entidades reales round-trip.

**Infinitas sin tinte (pedido 1):** el lavado violeta de página
(--inf-paper), el masthead-bloque sólido y el duotono violeta sobre TODA
la fotografía (hub + cover del artículo, .lect-duotone) se fueron; el
violeta queda como acento — regla gruesa, wordmark, cifras del marcador,
línea base bajo cada foto. Los tokens --inf-* ahora montan sobre el tema
del sitio, así que el hub respeta modo oscuro por primera vez (antes
forzaba papel claro).

**Sección Infinitas de portada (pedido 2) — rediseño "El Marcador en
portada":** components/sections/InfinitasSection.tsx reescrito (prefijo
infsec-*; el inf-* viejo, tarjetas con overlay negro y punto VERDE,
eliminado de sections.css/responsive/portada). Lead editorial partido
(foto limpia con línea base violeta, titular serif) + lista lateral
numerada estilo marcador con reglas FT; wordmark violeta en minúsculas
como marca de sección (si el heading del CMS es "Infinitas", el h2 mismo
viste el wordmark — sin tartamudeo de marca). HomeChoreography excluye
.infsec-head del barrido verde (la sección se enmarca sola en violeta).

**El Archivero en movimiento (pedido 5):**
components/products/LanaArchiveCabinet.tsx — el grid de folders de
/la-lana ahora es un archivero scroll-driven: runway + stage sticky
100vh, cajón dibujado en CSS (frente, jaladera, placa, ranura oscura),
timeline con scrub que saca cada expediente del cajón (el anterior se
hunde y se desvanece — un archivo devuelto, el siguiente jalado), placa
de progreso "EXP. NNN / NNN". Tres modos: sin JS/reduced-motion → grid
plano server-rendered (cero CLS, base real del markup); móvil → cada
folder se desliza fuera del labio del cajón al entrar al viewport, sin
secuestrar el scroll; desktop → el archivero completo. Solo
transform/opacity (compositor). Iterado sobre capturas: el arranque
yPercent apilaba los folders a media pantalla (porcentaje de la ALTURA
del folder, no del viewport) — corregido a unidades vh.

**Ocho dispositivos nuevos (encargo de mitad de sesión).** No existía
ninguna lista de "8 pendientes" en el repo (los 13 documentados ya
estaban implementados) — se diseñaron ocho formas nuevas al estándar de
la casa: `Termómetro` (avance hacia meta, mismo-unit obligatorio),
`Contrato` (term sheet con líneas de firma), `Ranking` (barras solo si
las unidades coinciden), `Votación` (tally semántico con umbral),
`Calendario` (la agenda, contraparte de Cronología), `Perfil` (ficha con
monograma), `Escala` (un número contra sus varas de medir), `Reloj`
(cuenta regresiva computada en el server por render). Parser + builder +
registro en DEVICES + CSS en lectura.css montado en
--lect-accent/--lect-ink-accent (las cuatro pieles de producto gratis) +
hook de motion genérico nuevo `[data-lect-grow]` en ArticleMotion.
Malformado queda inerte (probado). Documentados en
dynamic-element-library.md (13→21 en librería, checklist, ambos SKILL.md,
_GOVERNANCE, la-lana-article-spec). Sampler visual claro/oscuro revisado.

**Verificación de la sesión completa:** `npx tsc --noEmit` y `npx eslint`
limpios; `next build` limpio; suite Playwright propia (scratchpad)
15/15 — tema pre/post hidratación, tormentas de clics del héroe,
sección/hub Infinitas (regla violeta, punto violeta, masthead
transparente, fotos sin filtro, hub oscuro real), archivero
(desktop is-cabinet + progreso avanza, móvil en grid), cero anchors
anidados. Los dispositivos: 8/8 renderizan, 4/4 malformados inertes,
capturas en ambos temas.

**Pendiente:** (1) el paso post-deploy de la migración de source (arriba);
(2) los placeholders legales siguen siendo los de siempre — NO se
tocaron, bloqueados en datos de negocio; (3) la skill `verify` está
desactualizada de gala (describe el sitio estático pre-Next y un sandbox
Linux) — observación logueada, reescribirla en la próxima revisión de
skills; (4) si el usuario tenía OTRA lista de 8 elementos en mente,
decirlo y se construyen esas.
