...
0a. **Look at what the blocked link is carrying before treating it as blocked.**
   ...
   posting news to X usually attaches its communiqué as IMAGES, and those are
   the full primary source. On that run the two attachments were the complete
   two-page press release: the announcement's own bullets, both presidents'
   quotes and the implementation date, with no secondary outlet involved. The
   sequence is `cdn.syndication.twimg.com/tweet-result?id=<id>&token=a` for the
   tweet JSON, `mediaDetails[].media_url_https` for the asset paths, `curl` them
   to the scratchpad, read them. The same applies anywhere a post, a
   PDF-as-image or a screenshot **is** the document.

0b. **Ask the human in this session for the article text.** (2026-08-20, the
   FIFA Sub-15 run.) NYT and The Athletic reliably return "unable to fetch", and
   a subscriber often has direct access to exactly the primary this skill
   cannot reach. On that run the secondary-sourced first draft got the broad
   strokes right and missed load-bearing details only the primary had — that
   FIFA never told its own Council about the investment talks, Infantino's own
   December quote to contrast against it, Boehly's full title. The human
   volunteered the text unprompted *after* seeing that draft, which cost a full
...
**When every rung fails, the story does not get drafted.** (Closes an open gap
in this ladder: it defined four rungs and no bottom, so a run that exhausted
them had no written instruction and improvised.) If nothing loads, nobody
syndicated it, there is no release, and two independent outlets do not agree on
the figures, then the only sourcing left is one outlet's framing, which is what
this whole funnel exists to avoid. Say so to the human and stop. A story held
for a day until a second source appears costs nothing; a story published on one
unverified outlet is the failure mode `overlap-check.md` and Step 2 are both
built to prevent.

**The fallback has to be visible in the artifact, not just in the run report.**
A run report vanishes with the session. When a rung below 0b was used, the
draft says so in the prose the reader sees ("según reportó X", the transparency
line, or the `Fuentes:` line naming who actually carried the fact) — otherwise
an article built from secondaries is indistinguishable afterwards from one built
from the primary, and the next audit cannot tell them apart.
...
There is no separate "wire story" entry in `KNOWN_SOURCES` / `SOURCE_LABELS` to
reach for, and none is needed.

**The machine key is `"noticias"`, never `"industry-shots"`** (retired
2026-08-14, `fields-and-taxonomy.md` → "`publication` / `source`"). This file
said `"industry-shots"` until 2026-08-25 because it was deleted before the key
was retired and restored after, and every sourced-article run in between filed
into the legacy bucket: 117 rows carry `source='industry-shots'` and only 54 of
them have a `substackUrl`, so the section a piece belongs to and the funnel that
produced it are no longer separable in the archive. `fields-and-taxonomy.md`
owns this pair; if the two ever disagree again, that file wins.
