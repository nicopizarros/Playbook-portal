...
# Publish Newsletter — Substack link → live article, no human in the loop
...
→ `<pub>.substack.com` redirect automatically — re-fetch the redirect URL it
...
- **sourceUrl** — a unique per-item dedupe key: `` `${substackUrl}#<slug-of-title-or-topic>` ``. This is what the DB's unique index dedupes on (`articles.sourceUrl`) — re-running this skill on the same link will no-op on already-inserted stories instead of duplicating them.
