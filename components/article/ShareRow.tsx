...
//
// Roadmap Agosto 2026, Fase 3: grew from WhatsApp/X only to also cover
// Facebook (broad reach in Mexico) and LinkedIn (Playbook's B2B sports-
// business readership), plus a copy-link fallback for any channel not
// listed here (Telegram, email, SMS, ...). Same no-SDK share-URL pattern
// for all three new ones, nothing added needs its own account/API key.
export function ShareRow({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="share-row" role="group" aria-label="Compartir este artículo">
      ...
      <a className="share-btn" href={fbHref} target="_blank" rel="noopener noreferrer" aria-label="Compartir en Facebook" onClick={pulse}>
      ...
      <a className="share-btn" href={liHref} target="_blank" rel="noopener noreferrer" aria-label="Compartir en LinkedIn" onClick={pulse}>
        <svg viewBox="0 0 24 24" width="13" height="13" aria-hidden="true">
          <path
            fill="currentColor"
